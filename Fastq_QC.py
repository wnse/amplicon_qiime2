...
def cal_q20(f, threads=1, max_seq_per_t=1000000):
    ...
    # logging.info(f)
    # logging.info(total_reads, total_base, q20/total_base, q30/total_base, gc/total_base)
...
def Fastq_QC(fq_list, outdir, threads):
    ...
    try:
        ...
        out_file_list['qc_out_dir'] = qc_out_dir
    except Exception as e:
        logging.error(e)
    try:
    ...
    mkdir(tmpdir)
    trime_out, fq_trime_list = run_trim_pe(fq_list, tmpdir, threads)
    if trime_out:
    ...
    #     genomescope_out, genomescope_res , genomescope_png = run_genomescope(histo_file, tmpdir, read_length)
    #     if genomescope_res:
...
if __name__ == '__main__':
    ...
    try:
        ...
        with open(os.path.join(outdir, 'Fastqc.json'), 'w') as H:
            json.dump(out_file_list_tmp, H, indent=2)
        s = f'Fastqc\tD\t'
    except Exception as e:
